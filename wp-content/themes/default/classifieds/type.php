...
<?php $results = $wp_query->data['type']; global $gRating;  ?> 
<?php $num = Property::getCountType($results[0]->type_slug); $headings = $results[0]->type_name; ?>
<div id="content">
				...
				<?php foreach ($results as $row) {?>
					<li>
						<div class="hme-section">
							<a href="<?php echo Property::getPermalink($row->slug);?>"><img src="<?php echo Site::getThumbUrl($row->image,116,87); ?>" alt="<?php echo $row->title; ?>" title="<?php echo $row->title; ?>" /></a>
							<h3><a href="<?php echo Property::getPermalink($row->slug);?>"><?php echo $row->title; ?></a></h3>
							<p><?php if($row->no_bedrooms){ echo  $row->no_bedrooms." Bedroom,";}if($row->area){echo $row->area." sq.ft,"; } echo $row->type_name; ?></p>
						...
						<?php } ?>
							...
							<p class="call-back"><a href="javascript:;" onclick="Site.showDialog('callback-popup<?php echo $row->id; ?>', 'requestcallback-pop.php?property_id=<?php echo $row->id; ?>&agent_id=<?php echo $row->agent_id;?>', 'login-ind');">Request call back</a></p>
						</div>
